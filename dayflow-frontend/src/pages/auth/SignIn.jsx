...
export default function SignIn() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      await signIn(form.loginId.value.trim(), form.password.value)
      navigate('/employees')
    } catch (err) {
    ...
    <div className="flex min-h-screen bg-base-bg">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col items-center justify-center bg-base-panel border-r border-base-border px-16 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <h2 className="mb-3 text-2xl font-semibold text-white">Human Resource Management</h2>
          <p className="max-w-sm text-sm leading-relaxed text-slate-400">
            Streamline attendance, payroll, and leave management — all in one beautiful, unified platform.
          </p>
          <div className="mt-10 flex flex-wrap justify-center gap-2">
            {['Attendance Tracking', 'Payroll', 'Leave Management', 'Employee Profiles'].map(f => (
              <span key={f} className="rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-xs text-accent">{f}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12">
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="loginId" className="mb-1.5 block text-xs font-medium text-slate-300">Login ID / Email</label>
              <input
                id="loginId"
                name="loginId"
                type="text"
                required
                placeholder="e.g. DF26JD0001 or you@company.com"
