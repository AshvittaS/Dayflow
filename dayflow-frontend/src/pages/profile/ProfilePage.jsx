...
export default function ProfilePage() {
  ...
  // Fetch employee data from real API
  ...
  if (error || !viewedEmployee) return (
    <div className="flex flex-col items-center justify-center py-32 text-center">
      <p className="text-sm font-medium text-slate-400">{error || 'Employee not found.'}</p>
      <button onClick={() => navigate('/employees')} className="mt-4 text-xs text-accent hover:underline">← Back to Employees</button>
    </div>
  )

  const initials = viewedEmployee.name?.split(' ').map(n => n[0]).join('') || '?'

  return (
    <div>
      {!isOwnProfile && (
        <button onClick={() => navigate('/employees')} className="mb-5 flex items-center gap-1 text-xs text-slate-500 hover:text-accent">
      ...
      <div className="mb-6 flex flex-wrap items-end gap-5">
        <div className="relative">
          ...
          <span className="absolute bottom-0.5 right-0.5 h-4 w-4 rounded-full border-2 border-base-bg">
            {viewedEmployee.status === 'leave'
              ...
              : <span title={viewedEmployee.status} className={`block h-full w-full rounded-full ${viewedEmployee.status === 'present' ? 'bg-status-present' : 'bg-status-absent'}`} />
            }
          </span>
        </div>
        <div className="flex-1">
          ...
          {viewedEmployee.loginId && <p className="mt-0.5 text-xs text-slate-600">ID: {viewedEmployee.loginId}</p>}
        </div>
        {!isOwnProfile && (
          <span className="rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-xs font-medium text-accent">View-only mode</span>
        )}
      </div>
      ...
      <div className="mb-6 flex gap-0 border-b border-base-border">
        {visibleTabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-5 py-2.5 text-sm font-medium transition-colors ${tab === t ? 'border-b-2 border-accent text-white' : 'text-slate-500 hover:text-slate-300'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === 'Resume' && (
        <div className="space-y-6">
          ...
          {viewedEmployee.about && (
            <Section title="About"><p className="text-sm text-slate-300 leading-relaxed">{viewedEmployee.about}</p></Section>
          )}
          {viewedEmployee.skills?.length > 0 && (
            <Section title="Skills">
              <div className="flex flex-wrap gap-2">
                {viewedEmployee.skills.map(s => <span key={s} className="rounded-full bg-accent/10 px-3 py-1 text-xs text-accent border border-accent/20">{s}</span>)}
              </div>
            </Section>
          )}
          {viewedEmployee.certifications?.length > 0 && (
            <Section title="Certifications">
              <ul className="space-y-1.5">
                {viewedEmployee.certifications.map(c => <li key={c} className="flex items-center gap-2 text-sm text-slate-300"><span className="h-1.5 w-1.5 rounded-full bg-accent" />{c}</li>)}
              </ul>
            </Section>
          )}
          {viewedEmployee.interests?.length > 0 && (
            <Section title="Interests">
              <div className="flex flex-wrap gap-2">
                {viewedEmployee.interests.map(i => <span key={i} className="rounded-full border border-base-border bg-base-card px-3 py-1 text-xs text-slate-400">{i}</span>)}
              </div>
            </Section>
          )}
        </div>
      ...
      {tab === 'Salary Info' && isAdmin && <SalaryInfoTab employeeId={empId} />}
    </div>
  )
}
